...
function LockIcon({ locked }: { locked: boolean }) {
    return locked ? (
        // Closed lock — keys loaded
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
            <path d="M7 11V7a5 5 0 0 1 10 0v4" />
        </svg>
    ) : (
        // Open lock — keys not ready
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
...
function KeyStatusIndicator() {
    const { keysReady, keyError } = useCrypto()
    const title = keysReady
        ? 'E2E keys loaded'
        : keyError
            ? `Key error: ${keyError}`
            : 'Initialising keys…'
    ...
    return (
        <span
            ...
            className={`
                ...
                ${keysReady
                    ? 'text-emerald-600 bg-emerald-500/10 ring-1 ring-emerald-500/20 dark:text-emerald-400 dark:bg-emerald-500/10'
                    : 'text-red-500 bg-red-500/10 ring-1 ring-red-500/20 dark:text-red-400'}
            `}
        >
            <LockIcon locked={keysReady} />
            <span className="hidden sm:inline">{keysReady ? 'E2E' : 'No Key'}</span>
        </span>
    )
}
                ...
                <div className="flex items-center gap-2">
                    ...
                    {user ? (
                        <div className="flex items-center gap-2.5">
                            {/* Key Status badge — only when logged in */}
                            <KeyStatusIndicator />
